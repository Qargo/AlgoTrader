...
#        self.sell_cost_pct = sell_cost_pct
        self.buy_fix_cost = buy_fix_cost
        self.sell_fix_cost = sell_fix_cost
        ...
        def _do_sell_normal():
            if self.state[index+1]>0: 
                ...
                if self.state[index+self.stock_dim+1] > 0:
                    ...
                    # change the sell_amount to reflect the fix price for the sell cost
                    sell_amount = self.state[index + 1] * (sell_num_shares - self.sell_fix_cost)
                    ...
                    self.cost += self.state[index + 1] * self.sell_fix_cost
                    self.trades+=1
                else:
                    sell_num_shares = 0
            else:
    ...
    def _buy_stock(self, index, action):
        ...
        def _do_buy():
            if self.state[index+1]>0: 
                ...
                buy_amount = self.state[index + 1] * (buy_num_shares + self.buy_fix_cost)
                self.state[0] -= buy_amount

                self.state[index+self.stock_dim+1] += buy_num_shares
                
                # self.cost+=self.state[index+1] * buy_num_shares * self.buy_cost_pct
                self.cost += self.state[index + 1] * self.buy_fix_cost
                self.trades+=1
            else:
                buy_num_shares = 0
